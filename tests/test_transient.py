...
class TestTransientSolver:
    ...
    def test_reaction_mass_conservation_at_half_c(self):
        """Solid Li change should match the interfacial Faradaic source at 0.5C."""
        net = create_cathode_network(
            shape=[5, 5, 5], spacing=1e-5, porosity=0.5, seed=42,
        )
        solver = TransientSolver(net, T=298.15)
        ...
        for _ in range(5):
            ...
            lhs = np.sum(vol_s * (c_new - c_old))
            reaction_mol_rate = 0.0
            for t, *_ in solver._interfaces:
                reaction_mol_rate += result["I_rxn"][t] * net["throat.area"][t] / F
            rhs = -dt * reaction_mol_rate

            total_lithium = max(np.sum(vol_s * c_old), 1e-30)
            err = abs(lhs - rhs)
            assert err / total_lithium < 0.01, (
                f"solid Li balance error too large: lhs={lhs:.4e}, rhs={rhs:.4e}"
            )
